...
function providerModelsQuery(
  ...
  const extraArgs = settings.data
    ? JSON.stringify(settings.data.provider_extra_args?.[provider] ?? [])
    : cached?.extraArgs ?? JSON.stringify([]);
  const initial = cached && cached.binaryOverride === binaryOverride && cached.extraArgs === extraArgs
    ? cached
    : undefined;
  return {
    queryKey: [
    ...
    queryFn: async () => {
      ...
      writeProviderProbeCache(persistentStorageSync(), address, provider, binaryOverride, extraArgs, data);
      return data;
    },
